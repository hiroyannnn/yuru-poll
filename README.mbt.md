...
![デモ: 左が司会者画面、右が参加者のスマホ画面。発言が確率分布になり、小数票として集計される](docs/demo.gif)

左が司会者画面（`/`）、右が参加者のスマホ画面（`/join`）です。ローカルの open-jev（Gemma 3 4B）に実際に判定させた約 40 秒の録画で、字幕だけ録画時に重ねています。
「スプラ見たい気もするし見たくない気もする」がスプラ 56% / どれでもない 44% に分かれて 0.6 票になり、言い直すとその人の票が移る様子が映っています。
